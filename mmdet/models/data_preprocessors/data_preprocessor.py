...
from mmdet.utils import stack_batch
...
class DetDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        # print("[DEBUG] Data Preprocessor: data_samples type:", type(data_samples))
        # print("[DEBUG] Data Preprocessor: data_samples content:", data_samples)
...
@MODELS.register_module()
class GeneralDataPreprocessor(ImgDataPreprocessor):
    """Image pre-processor for detection tasks.
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        """Get the pad_shape of each image based on data and
        pad_size_divisor."""
        # print('_get_pad_shape',type(data),data.keys())
        _batch_inputs = data['inputs']
        # Process data with `pseudo_collate`.
        if is_seq_of(_batch_inputs, torch.Tensor):
            batch_pad_shape = []
    ...
    def pad_gt_sem_seg(self,
                       batch_data_samples: Sequence[DetDataSample]) -> None:
        """Pad gt_sem_seg to shape of batch_input_shape."""
        # 调试打印：检查传入 batch_data_samples 的类型和内容
        # print("[DEBUG] pad_gt_sem_seg: batch_data_samples type:", type(batch_data_samples))
        # print("[DEBUG] pad_gt_sem_seg: batch_data_samples content:", batch_data_samples)
        try:
        ...
        try:
            if 'gt_sem_seg' in batch_data_samples[0]:
                for data_samples in batch_data_samples:
                    try:
                        gt_sem_seg = data_samples.gt_sem_seg.sem_seg
                        gt_key_is_semseg = True
                    except Exception as e:
                        # print("[DEBUG] pad_gt_sem_seg: Exception when accessing data_samples.gt_sem_seg.sem_seg:", e)
                        gt_sem_seg = data_samples.gt_sem_seg.data
                        gt_key_is_semseg = False
