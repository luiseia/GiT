...
class GiT(BaseDetector, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 support_tasks: List = ['detection', 'semantic_segmentation', 'instance_segmentation', 'caption', 'grounding', 'occupancy_prediction' ],
    ...
    def forward_visual_modeling(self, batch_inputs: Tensor,
        ...
        """
        # 在 forward_visual_modeling 开始处确保输入在正确设备上
        batch_inputs = batch_inputs.to(self.backbone.pos_embed.device)
    ...
    def forward_transformer(self, patch_embed: Tensor, patch_mask: Tensor, patch_resolution: Tuple,
        ...
        assert not (self.global_only_image and self.backbone.layers[0].window_size <= 0), \
            'the first layer does not support global only image.'
        ...
        if self.mode not in ['caption'] :
            ...
            if self.mode == 'semantic_segmentation':
                ...
                tasks_tokens_embedding = torch.cat([tasks_tokens_embedding[num_classes+1:],
                                                    tasks_tokens_embedding[:num_classes+1]], dim=0)
        else:
        ...
        elif self.mode in ['detection', 'semantic_segmentation', 'instance_segmentation', 'caption', 'occupancy_prediction']:
        ...
        if self.training:
            ...
            for data_sample in batch_data_samples:
                ...
                elif self.mode in ['semantic_segmentation']:
            ...
            select_grid_reference = torch.gather(grid_reference, 1, batch_select_index[:, :, None].repeat(1, 1, grid_reference.shape[-1]))
            ...
            for layer_id, layer in enumerate(self.backbone.layers):
                grid_feature = self.get_grid_feature(patch_embed, select_grid_reference,
                                                    select_grid_start_embed, select_grid_token[:, :, 0, :], layer_id)
                ...
                if self.use_checkpoints:
                    if self.global_only_image and layer.window_size <= 0:
                        ...
                        patch_embed, text_embed, select_grid_token = checkpoint.checkpoint(
                            layer, patch_embed, seq_embed, select_grid_reference, text_embed,
            ...
            output_dict = {
                'all_layer_pred_seq_logits': all_layer_pred_seq_logits,
            ...
            if self.mode in ['detection', 'instance_segmentation', 'semantic_segmentation', 'grounding', 'occupancy_prediction']:
                ...
                output_dict = self.task_specific_heads[self.mode+'_head'].decoder_inference(
                    ...
                    self.task_embedding[self.mode_id], tasks_tokens_embedding,
                    self.grid_interpolate, self.global_only_image, self.tokenizer)
            else:
                raise NotImplementedError
            return output_dict
