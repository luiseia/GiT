...
class OCCHead(BaseModule):
    ...
    def __init__(self,
        ...
        # Build assigner based on BEV occupancy ground truth bbox matching.
        if train_cfg:
            assert 'assigner' in train_cfg, 'assigner must be provided in train_cfg.'
            assigner = train_cfg['assigner']
            self.assigner = TASK_UTILS.build(assigner)
            if train_cfg.get('sampler', None) is not None:
                raise RuntimeError('OCC head does not use a sampler.')
    ...
    def _get_targets_single_based_on_bev(self, bev_bbox_gt: Tensor,
        ...
        bev_H, bev_W = img_meta['bev_shape']  # 如 (200, 200)
        
        # 模拟生成候选的 grid 位置，这里假设已有 self.num_queries 个候选位置，
        # 每个位置在 BEV 空间归一化坐标下 (cx, cy) ∈ [0,1]
        # 实际中候选位置可以来自于特定设计，例如均匀采样或学习得到的 grid.
        reference_pred = torch.rand(self.num_queries, 2, device=bev_bbox_gt.device)
    ...
    def decoder_inference(self, 
        ...
        # Optionally: For BEV occupancy bbox, further apply a learnable BEV mapping
        # 使得 token输出更符合 BEV空间特征，这里假设输出维度不变
        outputs_coords = self.bev_mapping(outputs_coords)
    ...
    def _predict_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        bbox_index = indexes // (self.num_classes + 1)
