...
def grid_window_partition(x: torch.Tensor, grid_position: torch.Tensor,
    ...
    num_win_x, num_win_y = (torch.div(image_pad_hw[1], window_size,rounding_mode='trunc'  # or 'trunc'
    ), torch.div(image_pad_hw[0] , window_size,rounding_mode='trunc'))
    num_wins = num_win_x * num_win_y
    win_coors_x = torch.div(grid_position[:, :, 0] * image_pad_hw[1], window_size, rounding_mode='trunc') # bs, sampled_grid_num
    win_coors_y = torch.div(grid_position[:, :, 1] * image_pad_hw[0], window_size, rounding_mode='trunc') # bs, sampled_grid_num
    ...
    token_per_win = torch.div(sorted.shape[0], num_wins, rounding_mode='trunc')
