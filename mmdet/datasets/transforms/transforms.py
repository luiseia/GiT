...
@TRANSFORMS.register_module()
class SegPhotoMetricDistortion(BaseTransform):
    """Apply photometric distortion to image sequentially, every transformation
    is applied with a probability of 0.5. The position of random contrast is in
    second or second to last.
...
@TRANSFORMS.register_module()
class MMCVRandomFlip(MMCV_RandomFlip):
    """Flip the image & bbox & mask & segmentation map. Added or Updated keys:
    flip, flip_direction, img, gt_bboxes, and gt_seg_map. There are 3 flip
    modes:
...
@TRANSFORMS.register_module()
class Mask2DenseContour(BaseTransform):
    """Transform gt mask to dense polygon contours.
    ...
    def __init__(self, restore_all=False) -> None:
        super().__init__()
        self.restore_all = restore_all

    def transform(self, results: Dict) -> Dict:
        gt_masks = results['data_samples'].gt_instances.masks.masks

        contours = self.mask2contour(gt_masks)
        results['data_samples'].gt_instances['contours'] = contours

        return results
    
    def mask2contour(self, gt_masks):
        contours = []
        for mask in gt_masks:
            contour, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
            contour = list(contour)
            if self.restore_all:
                count = np.concatenate([c[:, 0, :] for c in contour])
            else:
...
@TRANSFORMS.register_module()
class AddMetaInfo(BaseTransform):
    """
    Add task-related meta information to the input results.
    
    本模块将 meta_dict 中的所有信息更新到结果中，并根据 task_name 计算 vocab list，
    将生成的词汇列表保存到 meta_dict['git_cfg']['use_vocab_list'] 中。
    
    对于 'occupancy_prediction' 任务，也按照检测任务的方式生成词汇列表。
    """
    def __init__(self, meta_dict=None):
        self.meta_dict = meta_dict

    def transform(self, results: dict) -> dict:
        # 将 meta_dict 中所有信息更新到 results 中
        if self.meta_dict is not None:
            results.update(self.meta_dict)
            # print("After AddMetaInfo, keys:", list(results.keys()))
        else:
            print('self.meta_dict is None')

        task_name = self.meta_dict.get('task_name', None)
        if task_name is None:
            raise ValueError("meta_dict 中必须包含 'task_name' 字段")

        # 尝试从 results 中获取类别列表，如果没有则使用空列表
        if 'classes' in results:
            classes = list(results['classes'])
        else:
            classes = []

        # 对于 detection、instance_segmentation 以及 occupancy_prediction
        # 任务，生成的词汇列表为：类别列表 + ['background'] + 数字字符串（依据 head_cfg 中 num_bins）
        if task_name in ['detection', 'instance_segmentation']:
            num_bins = self.meta_dict.get('head_cfg', {}).get('num_bins', 0)
            use_vocab_list = classes + ['background'] + [str(i) for i in range(num_bins + 1)]
        elif task_name == 'semantic_segmentation':
            use_vocab_list = classes + ['background', 'background1', 'background2']
        ...
        # 将生成的词汇列表写入 git_cfg 中
        if 'git_cfg' in self.meta_dict:
            self.meta_dict['git_cfg']['use_vocab_list'] = use_vocab_list
        else:
            self.meta_dict['git_cfg'] = {'use_vocab_list': use_vocab_list}
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(meta_dict={self.meta_dict})"
