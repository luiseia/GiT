...
import mmcv
import numpy as np
from mmdet.structures import DataSample  # 用于包装数据样本

@TRANSFORMS.register_module()
class PackOccInputs(BaseTransform):
    """将流水线输出打包成 occupancy 任务所需的格式。

    本模块收集上游流水线输出中的指定元信息（meta_keys），
    并将主要输入数据存放在 "inputs" 键中，最终构成包含 "inputs" 和
    "data_samples" 的字典。

    对于 occupancy 任务，假设真值（例如 gt_segmentation, gt_instance,
    gt_centerness, gt_offset, gt_flow, gt_backward_flow, gt_future_boxes,
    gt_future_labels）已经由上游流水线生成，最后会将这些信息放入
    data_sample 中，同时增加 'img_shape' 元信息，方便后续 transformer 等模块使用。
    """
    def __init__(self, meta_keys=('occ_future_ann_infos', 'task_name', 'head_cfg', 'git_cfg')):
        self.meta_keys = meta_keys
        # print("[DEBUG] PackOccInputs module loaded with meta_keys:", self.meta_keys)

    def transform(self, results: dict) -> dict:
        packed_results = dict()
        # 处理图像输入：如果存在 'img' 则转换为 tensor，并将其 shape 记录下来
        if 'img' in results:
            img = results['img']
            if len(img.shape) < 3:
                img = np.expand_dims(img, -1)
            if not img.flags.c_contiguous:
                ...
                img = to_tensor(img).contiguous()
            packed_results['inputs'] = img
            # 注意：这里我们取 tensor 的形状 (C, H, W)，故 img_shape 为 (H, W)
            results['img_shape'] = tuple(img.shape[1:])
        else:
            # 如果没有图像，则使用一个 dummy tensor，同时设置默认尺寸
            dummy = torch.zeros(3, 224, 224)
            packed_results['inputs'] = dummy
            results['img_shape'] = (224, 224)
        
        # 创建一个 DataSample 用于包装 occ 任务的真值和 meta 信息
        data_sample = DataSample()
        # 将 occupancy 任务的真值存入 data_sample（可根据需要扩展）
        import os
        ...
        for key in self.meta_keys:
            if key in results:
                img_meta[key] = results[key]
        # 同时加入 img_shape 信息
        img_meta['img_shape'] = results['img_shape']
        data_sample.set_metainfo(img_meta)
        
        packed_results['data_samples'] = data_sample

        # print("[DEBUG] PackOccInputs: Final result keys:", list(packed_results.keys()))
        return packed_results
